#!/usr/bin/env bash
# ============================================================
# llama-serve.sh — Multi-model llama-server launcher
...
# ---- Model registry ----------------------------------------
# Each entry: "file|port|ctx|ngl|extra_args"
# Categories inferred from flags: --embedding => embedding, etc.
declare -A REGISTRY=(
  # === Main chat models ===
  [lfm-8b-base]="$MODELS_DIR/8B-A1B-Base-GGUF/LFM2.5-8B-A1B-Base-Q6_K.gguf|8090|65536|99|--alias lfm2.5-8b-a1b-base --temp 0.2 --top-k 80 --repeat-penalty 1.05"
  [lfm-8b]="$MODELS_DIR/LFM2.5-8B-A1B-GGUF/LFM2.5-8B-A1B-Q6_K.gguf|8080|65536|99|--alias lfm2.5-8b-a1b --temp 0.2 --top-k 80 --repeat-penalty 1.05" # --chat-template-kwargs {\"keep_past_thinking\":true}
  ...
  [lfm-1.2b]="$MODELS_DIR/LFM2.5-1.2B-Instruct-GGUF/LFM2.5-1.2B-Instruct-Q4_K_M.gguf|8081|128000|99|--alias lfm2.5-1.2b-instruct"
  [vibethinker-3b]="$MODELS_DIR/VibeThinker-3B-GGUF/VibeThinker-3B.Q5_K_M.gguf|8082|131072|99|--alias vibethinker-3b --reasoning auto"
  [fastcontext-rl]="$MODELS_DIR/FastContext-1.0-4B-RL-GGUF/FastContext-1.0-4B-RL.Q6_K.gguf|8083|65536|99|--alias fastcontext-4b-rl"
  [fastcontext-sft]="$MODELS_DIR/FastContext-1.0-4B-SFT-Q5_K_M/fastcontext-1.0-4b-sft-q5_k_m.gguf|8084|65536|99|--alias fastcontext-4b-sft"

  # === Embedding models ===
  [lfm-embedding]="$MODELS_DIR/Embedding-350M-GGUF/LFM2.5-Embedding-350M-F16.gguf|8085|8192|99|--alias lfm2.5-embedding-350m --embedding --pooling mean"
  [lfm-colbert]="$MODELS_DIR/ColBERT-350M-GGUF/LFM2.5-ColBERT-350M-F16.gguf|8086|8192|99|--alias lfm2-colbert-350m --rerank --pooling rank"

  # === Vision-language models ===
  [lfm2-vl-450m]="$MODELS_DIR/LFM2.5-VL-450M-GGUF/LFM2.5-VL-450M-Q8_0.gguf|8088|8192|99|--alias lfm2.5-vl-450m --mmproj $MODELS_DIR/LFM2.5-VL-450M-GGUF/mmproj-LFM2.5-VL-450m-Q8_0.gguf"
  [lfm2-vl-1.6b]="/home/thomas/.cache/huggingface/hub/models--LiquidAI--LFM2.5-VL-1.6B-GGUF/snapshots/0df8719db7180cedababc2bc589abfe5e8ebcd1f/LFM2.5-VL-1.6B-Q8_0.gguf|8089|32768|99|--alias lfm2.5-vl-1.6b --mmproj /home/thomas/.cache/huggingface/hub/models--LiquidAI--LFM2.5-VL-1.6B-GGUF/snapshots/0df8719db7180cedababc2bc589abfe5e8ebcd1f/mmproj-LFM2.5-VL-1.6b-Q8_0.gguf"
)
...
start_model() {
  ...
  if [[ "$ngl" -gt 0 ]]; then
    local free_mib=$(_vram_free_mib)
    local wgb=$(_est_weight_gb "$file")
    local free_str
    if [[ "$free_mib" == "-1" ]]; then
